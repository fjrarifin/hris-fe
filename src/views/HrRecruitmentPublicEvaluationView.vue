<template>
  <div class="public-portal-container min-h-screen bg-slate-900 flex items-center justify-center p-4">
    <div class="portal-card" :class="{ 'portal-card--compact': loading || error || submitted || evaluationData?.submitted_at }">
      <div class="portal-header text-center mb-8">
        <img src="/hompimplay_icon.png" alt="Logo" class="brand-logo-public" />
        <div class="logo-placeholder inline-block bg-gradient-to-r from-primary to-blue-600 text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-3 shadow-md shadow-primary/20">
          HRIS Portal
        </div>
        <h1 class="text-2xl font-bold text-white tracking-tight">Formulir Evaluasi Wawancara User</h1>
        <p class="subtitle text-slate-400 mt-2 text-sm" v-if="candidateName">
          Calon pelamar: <span class="highlight">{{ candidateName }}</span> — Posisi <span class="highlight">{{ vacancyTitle }}</span> (Tahap {{ round }})
        </p>
      </div>

      <div v-if="loading" class="loading-state text-center py-12">
        <div class="spinner border-4 border-primary/20 border-t-primary rounded-full w-10 h-10 mx-auto animate-spin mb-4"></div>
      ...
      <div v-else-if="error" class="error-state text-center py-12">
        ...
        <p>{{ error }}</p>
        <p class="state-note">Silakan hubungi tim HRD jika Anda masih perlu mengisi evaluasi ini.</p>
      </div>

      <div v-else-if="submitted || evaluationData?.submitted_at" class="success-state text-center py-12">
      ...
      <form v-else @submit.prevent="handleSubmit" class="portal-form space-y-6">
        <div class="instructions">
          ...
          <p><strong>Petunjuk pengisian:</strong> Berikan penilaian pada sembilan aspek kompetensi dengan skala 1–4. Lengkapi seluruh aspek, catatan umpan balik, dan rekomendasi.</p>
        </div>
          ...
          <div v-for="aspect in aspectsList" :key="aspect.key" class="assessment-card">
            ...
            <div class="grid grid-cols-4 gap-2">
              <label 
                ...
                :class="[
                  'score-option',
                  form[aspect.key] === score ? 'is-selected' : ''
                ]"
              >
                <input 
                  type="radio" 
                ...
                <span class="text-[9px] uppercase tracking-wider mt-0.5 font-bold">{{ scoreLabels[score] }}</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="recommendation-grid" role="radiogroup" aria-label="Rekomendasi kelulusan">
            <label class="recommendation-option recommendation-option--danger" :class="{ 'is-selected': form.interview_recommendation === 'tidak_disarankan' }">
              <input v-model="form.interview_recommendation" name="interview_recommendation" type="radio" value="tidak_disarankan" required />
              <span>Tidak Lulus</span>
            </label>
            <label class="recommendation-option recommendation-option--warning" :class="{ 'is-selected': form.interview_recommendation === 'dipertimbangkan' }">
              <input v-model="form.interview_recommendation" name="interview_recommendation" type="radio" value="dipertimbangkan" required />
              <span>Pertimbangan</span>
            </label>
            <label class="recommendation-option recommendation-option--success" :class="{ 'is-selected': form.interview_recommendation === 'disarankan' }">
              <input v-model="form.interview_recommendation" name="interview_recommendation" type="radio" value="disarankan" required />
              <span>Lulus</span>
            </label>
          </div>
        </div>
...
const aspectsList = [
  { key: 'interview_appearance', label: 'Penampilan / Kerapihan', desc: 'Kerapihan pakaian, kesopanan berpakaian, pembawaan diri.' },
  { key: 'interview_attitude', label: 'Sikap / Kepribadian', desc: 'Kesopanan berbicara, etika, kerendahan hati, respon emosi.' },
  { key: 'interview_communication', label: 'Kemampuan Komunikasi', desc: 'Kelancaran bicara, artikulasi, kejelasan menyampaikan ide/argumen.' },
  { key: 'interview_motivation', label: 'Motivasi Kerja', desc: 'Semangat, antusiasme terhadap posisi/pekerjaan, daya juang.' },
  { key: 'interview_initiative', label: 'Inisiatif', desc: 'Proaktif, ketangkasan berpikir solusi, kemandirian.' },
  { key: 'interview_teamwork', label: 'Kerjasama Tim', desc: 'Kemampuan berkolaborasi, toleransi perbedaan pendapat.' },
  { key: 'interview_domain_experience', label: 'Keahlian Bidang / Pengalaman', desc: 'Pemahaman teknikal, relevansi keahlian dengan posisi lamaran.' },
  { key: 'interview_general_knowledge', label: 'Pengetahuan Umum', desc: 'Wawasan umum, adaptabilitas logika dasar.' },
  { key: 'interview_growth_potential', label: 'Potensi Berkembang', desc: 'Kemauan belajar hal baru, growth mindset, adaptasi tren.' },
]
...
const totalScore = computed(() => {
  return Number(form.interview_appearance || 0) +
