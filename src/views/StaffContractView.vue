...
const activeContracts = computed(
  () => data.value?.own_contracts.filter((item) => item.state === 'active') || [],
)
...
async function openPreview(contract) {
  ...
  try {
    ...
    const bytes = Uint8Array.from(atob(response.content_base64), (character) =>
      character.charCodeAt(0),
    )
    preview.url = URL.createObjectURL(
      new Blob([bytes], { type: response.mime_type || 'application/pdf' }),
    )
...
<template>
  <section class="space-y-5">
    ...
    <div
      v-if="loading"
      class="rounded-2xl border border-default bg-default py-20 text-center shadow-sm"
    >
    ...
    >
      <div
        class="flex flex-col gap-4 border-b border-default px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"
      >
      ...
      <div v-if="tab === 'mine'" class="space-y-8 p-5 sm:p-6 lg:p-8">
        <section>
          <div class="mb-4 flex items-center gap-3">
            <span
              class="flex size-9 items-center justify-center rounded-xl bg-success/10 text-success"
            >
          ...
          <div class="space-y-3">
            ...
            <div
              ...
              class="rounded-2xl border border-dashed border-default py-10 text-center"
            >
        ...
        <section>
          ...
          <div
            v-if="historyContracts.length"
            class="relative space-y-4 pl-6 before:absolute before:bottom-6 before:left-[7px] before:top-6 before:w-px before:bg-default"
          >
            <div v-for="contract in historyContracts" :key="contract.id" class="relative">
              <span
                class="absolute -left-6 top-6 size-[15px] rounded-full border-4 border-default bg-muted"
              ></span>
              <StaffContractCard
                :contract="contract"
                :loading="preview.contractId === contract.id"
          ...
          <div
            v-else
            class="rounded-2xl border border-dashed border-default py-10 text-center text-sm text-muted"
          >
      ...
      <div v-else class="p-5 sm:p-6 lg:p-8">
        <div class="mb-5">
          ...
          <p class="mt-1 text-sm text-muted">
            Data ditampilkan hanya untuk bawahan dengan kontrak yang masih aktif.
          </p>
        </div>

        <div class="grid gap-4 xl:grid-cols-2">
          <article
            v-for="item in data.team_contracts"
            :key="item.employee.nik"
          ...
          >
            <div class="flex items-center gap-3 border-b border-default px-5 py-4">
              <span
                class="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 font-bold text-primary"
              >
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-0.5 truncate text-xs text-muted">
                  {{ item.employee.nik }} · {{ item.employee.position || '-' }}
                </p>
              </div>
          ...
          <div
            v-if="!data.team_contracts.length"
            class="col-span-full rounded-2xl border border-dashed border-default py-14 text-center"
          >
            ...
            <p class="mt-1 text-xs text-muted">
              Kontrak aktif bawahan langsung akan tampil di sini.
            </p>
          </div>
        </div>
      </div>
    ...
    >
      <button
        type="button"
        class="absolute inset-0 bg-inverted/70 backdrop-blur-sm"
