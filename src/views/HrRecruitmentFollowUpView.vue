...
const activeActions = computed(() =>
  (dashboard.value?.action_center || []).filter((action) => action.count > 0),
)

const allRows = computed(() =>
  activeActions.value.flatMap((action) =>
...
const filteredRows = computed(() => {
  ...
  return allRows.value.filter((row) => {
    const matchesSearch =
      !keyword ||
      [row.name, row.vacancy, row.detail, row.actionLabel, row.pic].some((value) =>
        String(value || '')
          .toLowerCase()
          .includes(keyword),
      )
...
const activeStageOptions = computed(() =>
  [...new Set(allRows.value.map((row) => row.stage))].filter(Boolean),
)

watch([search, selectedCategory, selectedStage], () => {
  currentPage.value = 1
})
...
function openCandidate(row) {
  router.push({
    ...
    query: { candidate: row.id, status: row.stage },
  })
}
...
function backToDashboard() {
  router.push({
    ...
    query: { ...dashboardQuery(), month: selectedMonth.value },
  })
}
  ...
  <section class="follow-up-page">
    <header class="follow-up-hero">
      <div>
        ...
        <p>
          Seluruh hambatan proses kandidat yang memerlukan tindakan HRD agar recruitment dapat
          berlanjut.
        </p>
      </div>
      <UButton
    ...
    <div class="summary-grid">
      <article>
        <span class="summary-icon summary-icon--amber"><UIcon name="i-lucide-list-todo" /></span>
        <p>
          Total Pekerjaan<strong>{{ dashboard?.summary?.pending_actions || 0 }}</strong>
        </p>
      </article>
      <article>
        <span class="summary-icon summary-icon--indigo"><UIcon name="i-lucide-layout-list" /></span>
        <p>
          Jenis Tindak Lanjut<strong>{{ activeActions.length }}</strong>
        </p>
      </article>
      <article>
        <span class="summary-icon summary-icon--teal"><UIcon name="i-lucide-users-round" /></span>
        <p>
          Kandidat Terdampak<strong>{{ uniqueCandidateCount }}</strong>
        </p>
      </article>
    </div>
    ...
    <article class="follow-up-card">
      <div class="toolbar">
        <label class="month-filter"
          ><span>Periode</span
          ><input v-model="selectedMonth" type="month" :max="currentMonth" @change="loadFollowUps"
        /></label>
        <label class="search-filter"
          ><span class="sr-only">Cari</span><UIcon name="i-lucide-search" class="size-4" /><input
            v-model="search"
            type="search"
            placeholder="Cari kandidat, pekerjaan, lowongan, atau PIC..."
        /></label>
        <select v-model="selectedCategory" aria-label="Filter jenis tindak lanjut">
          <option value="">Semua pekerjaan</option>
          <option v-for="action in activeActions" :key="action.key" :value="action.key">
            {{ action.label }} ({{ action.count }})
          </option>
        </select>
        <select v-model="selectedStage" aria-label="Filter tahapan kandidat">
          <option value="">Semua tahap</option>
          <option v-for="stage in activeStageOptions" :key="stage" :value="stage">
      ...
      <div class="result-heading">
        <div>
          <strong>{{ filteredRows.length }} pekerjaan</strong
          ><small>Klik baris untuk membuka detail kandidat pada pipeline.</small>
        </div>
        <span v-if="loading"
          ><UIcon name="i-lucide-loader-circle" class="size-4 animate-spin" /> Memuat data</span
        >
      </div>
        ...
        <table class="follow-up-table">
          ...
          <tbody>
            <tr
              ...
              :key="`${row.actionKey}-${row.id}`"
              tabindex="0"
              @click="openCandidate(row)"
              @keydown.enter="openCandidate(row)"
            >
              <td>
                <div class="follow-up-task-cell">
                  <span
                    class="task-icon"
                    :style="{ '--task-color': actionColors[row.actionColor] }"
                  >
              ...
              <td>
                ...
                ><small>{{ row.vacancy }}</small>
              </td>
              <td>{{ row.detail || 'Perlu ditindaklanjuti oleh HRD' }}</td>
              <td>
                <span class="stage-badge">{{ stageLabels[row.stage] || row.stage }}</span>
              </td>
...
.toolbar {
  display: grid;
  grid-template-columns: 150px minmax(240px, 1fr) minmax(210px, 0.55fr) minmax(165px, 0.4fr);
  gap: 10px;
  padding: 16px;
...
.follow-up-table th:nth-child(1) {
  width: 25%;
}
.follow-up-table th:nth-child(2) {
  width: 17%;
}
.follow-up-table th:nth-child(3) {
  width: 18%;
}
.follow-up-table th:nth-child(4) {
  width: 13%;
}
.follow-up-table th:nth-child(5) {
  width: 18%;
}
.follow-up-table th:last-child {
  width: 36px;
}
.follow-up-table td {
  padding: 11px 12px;
...
.follow-up-table td:nth-child(2) {
  color: var(--ui-text-highlighted);
}
.follow-up-table td:nth-child(2) strong,
.follow-up-table td:nth-child(2) small {
  display: block;
  overflow: hidden;
...
.task-icon {
  ...
  place-items: center;
  border-radius: 8px;
  background: color-mix(in srgb, var(--task-color) 14%, var(--ui-bg));
