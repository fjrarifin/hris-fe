  ...
  return [
    {
      ...
      note: `${summary.active_candidates} dari ${summary.total_candidates} masih dalam proses`,
    ...
    {
      label: 'Kandidat Baru',
      value: summary.new_candidates,
      note: `Applied di ${selectedPeriodLabel.value}`,
      icon: 'i-lucide-user-plus',
      tone: 'sky',
      action: () => openPipeline({ month: filters.month }),
    },
    {
      label: 'Lowongan Aktif',
      value: summary.open_vacancies,
      note: `${summary.approved_headcount} kebutuhan disetujui`,
      icon: 'i-lucide-briefcase-business',
      tone: 'violet',
      action: () => router.push({ name: 'hr-recruitment-vacancies' }),
    },
    {
      label: 'Berhasil Hired',
      value: summary.hired_candidates,
      note: `Hire rate ${summary.hire_rate}%`,
    ...
    },
    {
      ...
      tone: summary.pending_actions ? 'amber' : 'emerald',
      action: () => openAllFollowUps(),
    },
    {
      label: 'Join Bulan Terpilih',
      value: summary.joining_in_period ?? summary.joining_soon,
      note: `Tanggal mulai kerja di ${selectedPeriodLabel.value}`,
  ...
  return pipeline
    .map((stage, index) => {
      ...
      const stepConversion =
        prev && prev.reached > 0 ? Math.round((stage.reached / prev.reached) * 100) : null
    ...
    .map((step, index, arr) => {
      ...
      return {
        ...step,
        isBottleneck: step.dropOff !== null && step.dropOff === maxDrop && maxDrop > 0,
      }
    })
})
...
const sortedEvaluationCandidates = computed(() =>
  [...(dashboard.value?.interview_quality?.candidates || [])].sort(
...
const documentPhases = computed(() => {
  ...
  const phaseMap = {
    ...
    case_submission: 'pre',
    user_summary: 'post',
    user_evaluation: 'post',
    reference_summary: 'post',
    offering_letter: 'closing',
    pkb: 'closing',
    onboarding: 'closing',
  }
  const phases = {
...
function formatPeriod(value) {
  ...
  return new Intl.DateTimeFormat(
    'id-ID',
    value.length === 7 ? { month: 'short', year: 'numeric' } : { day: 'numeric', month: 'short' },
  ).format(date)
}
  ...
  <section class="recruitment-dashboard space-y-5">
    <header class="dashboard-hero">
      <div>
        ...
        <p>
          Pantau funnel kandidat, pekerjaan tertunda, kelengkapan dokumen, dan kualitas proses
          rekrutmen.
        </p>
      </div>
    ...
    <form class="dashboard-filter" @submit.prevent="loadDashboard">
      <label>
        <span class="filter-label-with-tooltip">
          Periode Bulan
          <span
            class="filter-tooltip-icon"
            title="Data berdasarkan tanggal kandidat melamar (cohort Applied)"
          >
      ...
      <label>
        ...
        <select v-model="filters.vacancy_id">
          ...
          <option
            v-for="item in dashboard?.filter_options?.vacancies || []"
            :key="item.id"
            :value="item.id"
          >
            {{ item.title }}
      ...
      <label>
        ...
        <select v-model="filters.department">
          ...
          <option
            v-for="item in dashboard?.filter_options?.departments || []"
            :key="item"
            :value="item"
          >
            {{ item }}
      ...
      <label>
        ...
        <select v-model="filters.unit">
          ...
          <option v-for="item in dashboard?.filter_options?.units || []" :key="item" :value="item">
            {{ item }}
          </option>
        </select>
      </label>
      <label>
        ...
        <select v-model="filters.pic_nik">
          ...
          <option
            v-for="item in dashboard?.filter_options?.pics || []"
            :key="item.nik"
            :value="item.nik"
          >
            {{ item.name }}
      ...
      <label>
        ...
        <select v-model="filters.source">
          ...
          <option
            v-for="item in dashboard?.filter_options?.sources || []"
            :key="item"
            :value="item"
          >
            {{ item }}
    ...
    <template v-else-if="dashboard">
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            ...
            <div class="action-heading-controls">
              <span class="heading-badge heading-badge--warning"
                >{{ dashboard.summary.pending_actions }} tindakan</span
              >
              <UButton
                size="xs"
                variant="soft"
          ...
          <div v-if="candidateActionGroups.length" class="action-candidate-list">
            ...
            >
              ...
              <span
                ...
                :class="
                  daysSince(group.updated_at) >= 7
                    ? 'action-waiting--danger'
                    : daysSince(group.updated_at) >= 3
                      ? 'action-waiting--warn'
                      : ''
                "
              >
          ...
          <div v-else class="empty-state">
            <UIcon name="i-lucide-circle-check-big" class="size-8" /><strong
              >Semua proses terkendali</strong
            >
            <p>Tidak ada tindakan recruitment yang tertunda.</p>
          </div>
        </article>
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              ...
              <p>Jadwal wawancara HR dan user.</p>
            </div>
            <UButton
              size="xs"
              variant="soft"
              ...
              label="Lihat Semua"
              @click="openInterviewAgenda"
            />
          </div>
          <div v-if="dashboard.upcoming_schedules.length" class="schedule-list">
            <button
              v-for="item in dashboard.upcoming_schedules"
              :key="`${item.id}-${item.scheduled_at}-${item.type}`"
              type="button"
              @click="openPipeline({ candidate: item.id, status: item.stage })"
            >
              <span class="schedule-date"
                ><strong>{{ new Date(item.scheduled_at).getDate() }}</strong
                ><small>{{
              ...
              <span
                ...
                ><small>{{ item.type }} • {{ item.vacancy }}</small
                ><em>{{ formatDateTime(item.scheduled_at) }}</em></span
              >
              <UIcon name="i-lucide-arrow-up-right" class="size-4" />
            </button>
          </div>
          <div v-else class="empty-state empty-state--small">
            <UIcon name="i-lucide-calendar-x-2" class="size-7" /><strong>Belum ada agenda</strong>
            <p>Tidak ada wawancara dalam tujuh hari ke depan.</p>
          </div>
        </article>
      </div>
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              ...
              <p>
                Menunjukkan jumlah kandidat yang sudah mencapai setiap tahap dan kandidat yang masih
                diproses di tahap tersebut.
              </p>
            </div>
          ...
          <div v-if="dashboard.pipeline[1]" class="pipeline-reading-guide">
            ...
            <p>
              ...
              {{ dashboard.pipeline[1].reached }} dari
              {{ dashboard.summary.total_candidates }} kandidat sudah mencapai tahap
              {{ dashboard.pipeline[1].label }}; {{ dashboard.pipeline[1].current }} kandidat masih
              diproses di tahap tersebut.
            </p>
          </div>
            ...
            >
              ...
              <span class="pipeline-progress">
                <span class="pipeline-bar"
                  ><span
                    :style="{
                      width: `${(stage.reached / maxPipelineReached) * 100}%`,
                      background: stageVisuals[stage.key]?.color,
                    }"
                  ></span
                ></span>
                <small
                  >{{ stage.reached }} dari {{ dashboard.summary.total_candidates }} kandidat</small
                >
              </span>
              <span class="pipeline-count">
                <span
                  ><strong>{{ stage.current }}</strong
                  ><small> kandidat</small></span
                >
                <small>masih diproses</small>
              </span>
              <span
                class="pipeline-conversion"
                :title="`${stage.reached} dari ${dashboard.summary.total_candidates} kandidat sudah mencapai tahap ${stage.label}`"
                >{{ stage.conversion.toLocaleString('id-ID') }}%</span
              >
            </button>
          </div>
        </article>
        ...
        <article class="dashboard-card funnel-card">
          <div class="card-heading">
            <div>
              ...
              <p>
                Berapa kandidat yang gugur di setiap transisi tahap. Tahap dengan kehilangan
                terbesar adalah bottleneck rekrutmen.
              </p>
            </div>
          ...
          <div class="funnel-list">
            ...
            >
              ...
              <button
                ...
                class="funnel-row"
                :style="{
                  '--stage-color': stageVisuals[step.key]?.color,
                  '--stage-soft': stageVisuals[step.key]?.soft,
                }"
                ...
                <span class="funnel-pct-col">
                  ...
                  <template v-else-if="step.stepConversion !== null">
                    <span
                      ...
                      :class="
                        step.isBottleneck
                          ? 'funnel-pct-big--danger'
                          : step.stepConversion >= 70
                            ? 'funnel-pct-big--good'
                            : 'funnel-pct-big--warn'
                      "
              ...
              <div
                v-if="index < funnelSteps.length - 1 && step.dropOffCount > 0"
              ...
              >
                ...
                <span
                  ...
                  :class="
                    step.isBottleneck
                      ? 'funnel-dropout-chip--danger'
                      : step.dropOffCount >= 5
                        ? 'funnel-dropout-chip--warn'
                        : 'funnel-dropout-chip--mild'
                  "
                >
              ...
              <div
                v-else-if="index < funnelSteps.length - 1"
                class="funnel-connector funnel-connector--clean"
              >
                <span class="funnel-connector-line"></span>
              </div>
            </div>
          ...
          <section class="duration-section">
            ...
            <div class="duration-list">
              <div v-for="stage in dashboard.stage_durations.slice(0, 6)" :key="stage.key">
                ...
                ><strong>{{ formatHours(stage.average_hours) }}</strong>
              </div>
              <p v-if="!dashboard.stage_durations.length" class="empty-copy">
                Durasi tahap akan tampil setelah perpindahan tahap baru tercatat.
              </p>
            </div>
          </section>
        </article>
      ...
      <div class="dashboard-two-column">
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              <span class="section-kicker">Dokumen</span>
              <h2>Progress Kelengkapan</h2>
              <p>
                Dikelompokkan per fase. Completion dihitung dari kandidat yang sudah mencapai tahap
                terkait.
              </p>
            </div>
          </div>
            ...
            <div v-for="phase in documentPhases" :key="phase.key" class="doc-phase">
              ...
              >
                <span class="document-copy"
                  ...
                  ><small
                    >{{ item.complete }}/{{ item.eligible }} lengkap<span v-if="item.missing">
                      • {{ item.missing }} kurang</span
                    ></small
                  ></span
                >
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              <span class="section-kicker">Lowongan</span>
              <h2>Performa Lowongan</h2>
              <p>Volume kandidat dan pemenuhan kebutuhan per posisi.</p>
            </div>
            <UButton
              size="xs"
              variant="ghost"
              ...
              :to="{ name: 'hr-recruitment-vacancies' }"
            />
          </div>
            ...
            <table class="vacancy-table">
              <thead>
                <tr>
                  ...
                  <th>Aktif</th>
                  <th>Hired</th>
                  <th v-if="hasFulfillmentData">Pemenuhan</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="item in dashboard.vacancy_performance.slice(0, 7)"
                  :key="item.id"
                  @click="openPipeline({ vacancy_id: item.id })"
                >
                  <td>
                    <strong>{{ item.title }}</strong
                    ><small>{{
                      [item.department, item.unit].filter(Boolean).join(' • ') || 'Umum'
                    }}</small>
                  </td>
                  <td>{{ item.candidates }}</td>
                  <td>{{ item.active }}</td>
                  <td>{{ item.hired }}</td>
                  <td v-if="hasFulfillmentData">
                    <span v-if="item.fulfillment !== null" class="fulfillment"
                      ><i :style="{ width: `${Math.min(100, item.fulfillment)}%` }"></i
                      ><small>{{ item.fulfillment }}%</small></span
      ...
      <div class="dashboard-two-column">
        <article class="dashboard-card">
          <div class="card-heading">
            ...
            <span class="score-pill" title="Skor akumulatif dari 9 aspek, maks. 4 poin per aspek"
              ><small>Rata-rata</small>{{ dashboard.interview_quality.average_total_score
              }}<small>/36 poin</small></span
            >
          </div>
          <div class="evaluation-scope-summary">
            <span
              ><UIcon name="i-lucide-user-round-check" class="size-4" /><strong>{{
                dashboard.interview_quality.candidate_count
              }}</strong>
              kandidat</span
            >
            <span
              ><UIcon name="i-lucide-users-round" class="size-4" /><strong>{{
                dashboard.interview_quality.interviewer_count
              }}</strong>
              pewawancara</span
            >
            <span
              ><UIcon name="i-lucide-clipboard-check" class="size-4" /><strong
                >{{ dashboard.interview_quality.submitted }}/{{
                  dashboard.interview_quality.total
                }}</strong
              >
              evaluasi diisi</span
            >
          ...
          <div v-if="sortedEvaluationCandidates.length" class="evaluation-candidates">
            ...
            >
              <span
                ...
                ><small
                  >{{ candidate.vacancy }} • {{ candidate.evaluation_count }} evaluasi</small
                ></span
              >
              <span
                class="evaluation-candidate-score"
                :title="`${candidate.average_score} poin dari maks. 36 (9 aspek × 4)`"
              >
          ...
          <p v-else class="empty-copy">
            Belum ada evaluasi wawancara user yang diselesaikan pada filter ini.
          </p>
        </article>
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              <span class="section-kicker">Kompetensi</span>
              <h2>Rata-rata Aspek Kompetensi</h2>
              <p>Agregat nilai per aspek (maks. 4 poin) dari seluruh evaluasi yang diselesaikan.</p>
            </div>
          </div>
          <div class="aspect-list">
            <div v-for="aspect in interviewAspects" :key="aspect.key">
              <span>{{ aspect.label }}</span
              ><span class="aspect-bar"
                ><i :style="{ width: `${(aspect.average / 4) * 100}%` }"></i></span
              ><strong
                >{{ aspect.average
                }}<small style="font-weight: 400; font-size: 8px">/4</small></strong
              >
            </div>
          </div>
          <p v-if="!interviewAspects.length" class="empty-copy">
            Belum ada evaluasi yang diselesaikan pada filter ini.
          </p>
        </article>
      </div>
      ...
      <div class="dashboard-two-column">
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              <span class="section-kicker">Closing</span>
              <h2>Offering, PKB &amp; Onboarding</h2>
              <p>Progress kandidat setelah proses seleksi.</p>
            </div>
          </div>
          <div class="closing-stats">
            <div>
              <span class="closing-icon closing-icon--pink"
                ><UIcon name="i-lucide-file-signature"
              /></span>
              <p>
                Offering Ditandatangani<small
                  >{{ dashboard.offering_onboarding.offering.signed }} dari
                  {{ dashboard.offering_onboarding.offering.eligible }}</small
                >
              </p>
              <strong>{{ dashboard.offering_onboarding.offering.acceptance_rate }}%</strong>
            </div>
            <div>
              <span class="closing-icon closing-icon--amber"><UIcon name="i-lucide-stamp" /></span>
              <p>
                Persetujuan PKB<small
                  >{{ dashboard.offering_onboarding.pkb.signed }} signer selesai</small
                >
              </p>
              <strong>{{ dashboard.offering_onboarding.pkb.pending }} pending</strong>
            </div>
            <div>
              <span class="closing-icon closing-icon--green"
                ><UIcon name="i-lucide-user-round-check"
              /></span>
              <p>
                Onboarding Selesai<small
                  :title="'Proses administrasi onboarding dituntaskan — berbeda dengan join date resmi'"
                  >{{ dashboard.offering_onboarding.onboarding.imported }} sudah diimpor ke data
                  karyawan</small
                >
              </p>
              <strong>{{ dashboard.offering_onboarding.onboarding.completed }}</strong>
            </div>
          </div>
        </article>
        ...
        <article class="dashboard-card">
          <div class="card-heading">
            <div>
              <span class="section-kicker">Akuisisi</span>
              <h2>Sumber Kandidat</h2>
              <p>Distribusi sumber pelamar pada periode terpilih.</p>
            </div>
          </div>
          <div class="source-list">
            <div v-for="item in topSources" :key="item.source">
              <span
                ><strong>{{ item.source }}</strong
                ><small>{{ item.hired }} hired</small></span
              ><span class="source-bar"
                ><i :style="{ width: `${(item.count / maxSourceCount) * 100}%` }"></i></span
      ...
      <article class="dashboard-card activity-section">
        <div class="card-heading">
          <div>
            ...
            <h2>Aktivitas Recruitment Terbaru</h2>
            <p>
              Perubahan status dan tahap kandidat dalam periode filter. Log teknis sistem tidak
              ditampilkan di sini.
            </p>
          </div>
        </div>
        <div class="activity-list">
          <button
            v-for="item in meaningfulActivities"
            :key="item.id"
            type="button"
            @click="openPipeline({ candidate: item.candidate_id })"
          >
            <span class="activity-dot"></span
            ><span
              ><strong>{{ item.actor }}</strong>
              <p>{{ item.subject }}</p></span
            ><time>{{ formatDateTime(item.occurred_at) }}</time
            ><UIcon name="i-lucide-chevron-right" class="size-4" />
          </button>
          <p v-if="!meaningfulActivities.length" class="empty-copy">
            Belum ada perubahan status kandidat yang tercatat pada periode ini.
          </p>
        </div>
      </article>
    </template>
...
.recruitment-dashboard {
  --dash-border: var(--ui-border);
  --dash-bg: var(--ui-bg);
  --dash-muted: var(--ui-bg-muted);
...
.dashboard-hero {
  ...
  border-radius: 22px;
  background:
    radial-gradient(circle at 90% 0%, rgba(59, 130, 246, 0.16), transparent 36%),
    linear-gradient(135deg, #eff6ff, #fff 58%, #f8fafc);
...
.dashboard-filter {
  display: grid;
  grid-template-columns: repeat(8, minmax(100px, 1fr)) auto;
  align-items: end;
  gap: 10px;
  padding: 15px;
...
.pipeline-row {
  display: grid;
  grid-template-columns: 30px 120px minmax(80px, 1fr) 70px 48px;
  align-items: center;
  gap: 9px;
  width: 100%;
  padding: 5px;
...
.funnel-overall strong {
  font-size: 18px;
  line-height: 1;
}
.funnel-overall small {
  font-size: 10px;
  color: #60a5fa;
}
.funnel-bottleneck-badge {
  display: flex;
  align-items: center;
  gap: 5px;
  padding: 5px 9px;
...
.funnel-step--bottleneck .funnel-row {
  background: color-mix(in srgb, #f59e0b 6%, var(--dash-bg));
  border-color: color-mix(in srgb, #f59e0b 28%, var(--dash-border));
}
.funnel-row {
  display: grid;
  grid-template-columns: 26px minmax(80px, 1fr) 56px 52px 16px;
  align-items: center;
  gap: 8px;
  width: 100%;
  padding: 5px 6px;
...
.funnel-icon {
  ...
  place-items: center;
  border-radius: 7px;
  background: var(--stage-soft, #f1f5f9);
...
.funnel-reached-count {
  font-size: 12px;
  font-weight: 700;
  text-align: right;
  white-space: nowrap;
}
.funnel-reached-count small {
  font-size: 10px;
  color: var(--ui-text-dimmed);
  font-weight: 400;
}
...
.action-card {
  padding: 12px;
  border: 1px solid color-mix(in srgb, var(--action-color) 22%, var(--dash-border));
  border-radius: 12px;
  background: color-mix(in srgb, var(--action-color) 4%, var(--dash-bg));
}
.action-title {
  display: flex;
  align-items: center;
...
.action-title > span {
  ...
  place-items: center;
  border-radius: 8px;
  background: color-mix(in srgb, var(--action-color) 13%, var(--dash-bg));
...
.action-candidate {
  ...
  width: 100%;
  padding: 6px 2px;
  border-top: 1px solid color-mix(in srgb, var(--action-color) 12%, transparent);
  text-align: left;
  font-size: 9px;
}
...
.action-candidate-row {
  display: grid;
  grid-template-columns: minmax(100px, 1fr) minmax(0, 2fr) 80px 18px;
  align-items: center;
  gap: 10px;
  width: 100%;
  padding: 8px 10px;
...
.action-candidate-row:hover {
  border-color: #93c5fd;
  background: #eff6ff;
}
.action-candidate-info {
  display: flex;
  min-width: 0;
  flex-direction: column;
}
.action-candidate-info strong {
  font-size: 11px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.action-candidate-info small {
  color: var(--ui-text-dimmed);
  font-size: 10px;
  overflow: hidden;
  text-overflow: ellipsis;
...
.action-task-chip {
  ...
  padding: 2px 7px;
  border-radius: 999px;
  border: 1px solid color-mix(in srgb, var(--action-color) 35%, var(--dash-border));
  background: color-mix(in srgb, var(--action-color) 8%, var(--dash-bg));
  color: var(--action-color);
  font-size: 10px;
  font-weight: 700;
  white-space: nowrap;
...
.action-priority-row {
  ...
  border: 1px solid color-mix(in srgb, var(--action-color) 22%, var(--dash-border));
  border-radius: 11px;
  background: color-mix(in srgb, var(--action-color) 5%, var(--dash-bg));
  text-align: left;
  transition:
    background 0.15s,
    border-color 0.15s,
...
.action-priority-row:hover {
  border-color: color-mix(in srgb, var(--action-color) 50%, var(--dash-border));
  background: color-mix(in srgb, var(--action-color) 10%, var(--dash-bg));
  transform: translateY(-1px);
}
.action-priority-icon {
  display: grid;
  width: 38px;
  height: 38px;
  place-items: center;
  border-radius: 10px;
  background: color-mix(in srgb, var(--action-color) 15%, var(--dash-bg));
  color: var(--action-color);
}
...
.action-priority-copy strong,
.action-priority-copy small {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
...
.document-list button {
  display: grid;
  grid-template-columns: 175px minmax(80px, 1fr) 42px;
  align-items: center;
  gap: 12px;
  width: 100%;
  text-align: left;
...
.aspect-list > div {
  display: grid;
  grid-template-columns: 120px minmax(50px, 1fr) 28px;
  align-items: center;
  gap: 8px;
  font-size: 11px;
}
...
.source-list > div {
  display: grid;
  grid-template-columns: 95px minmax(50px, 1fr) 30px;
  align-items: center;
  gap: 8px;
}
...
.activity-list button > span:nth-child(2) {
  min-width: 0;
}
...
.skeleton-card,
.skeleton-panel {
  background: linear-gradient(90deg, var(--dash-muted), var(--dash-bg), var(--dash-muted));
...
@media (max-width: 640px) {
  ...
  .pipeline-row {
    grid-template-columns: 28px 90px minmax(50px, 1fr) 48px;
  }
  .pipeline-conversion {
    display: none;
  }
  ...
  .document-list button {
    grid-template-columns: 130px minmax(60px, 1fr) 37px;
  }
  .activity-list time {
    display: none;
  }
...
.dashboard-hero {
  ...
  background:
    radial-gradient(
      circle at 90% 0%,
      color-mix(in srgb, var(--dash-primary) 17%, transparent),
      transparent 38%
    ),
    linear-gradient(
      135deg,
...
.kpi-card--sky .kpi-icon {
  background: color-mix(in srgb, #0ea5e9 13%, var(--dash-surface));
  color: color-mix(in srgb, #0284c7 75%, var(--dash-text));
}
.kpi-card--violet .kpi-icon {
  background: color-mix(in srgb, #8b5cf6 14%, var(--dash-surface));
  color: color-mix(in srgb, #8b5cf6 76%, var(--dash-text));
}
.kpi-card--emerald .kpi-icon {
...
.aspect-bar i {
  background: linear-gradient(
    90deg,
    color-mix(in srgb, var(--dash-indigo) 65%, #ffffff),
    var(--dash-indigo)
  );
}
...
.source-bar i {
  background: linear-gradient(
    90deg,
    color-mix(in srgb, var(--dash-teal) 65%, #ffffff),
    var(--dash-teal)
  );
}
